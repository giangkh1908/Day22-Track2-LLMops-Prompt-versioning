...
def setup_vectorstore():
    ...
    """
    # TODO hoàn thành: Khởi tạo embeddings từ factory.
    embeddings = get_embeddings()

    # TODO hoàn thành: Đọc nội dung knowledge base.
    text = load_knowledge_base()

    # TODO hoàn thành: Chia text thành chunks với chunk_size=500, chunk_overlap=50.
    chunks = split_text(text, chunk_size=500, chunk_overlap=50)
    print(f"📚 Đã chia thành {len(chunks)} chunks")

    # TODO hoàn thành: Tạo FAISS vectorstore và trả về.
    vectorstore = build_vectorstore(chunks, embeddings)
    return vectorstore


# ── 2. RAG Prompt Template ─────────────────────────────────────────────────
# TODO hoàn thành: Tạo ChatPromptTemplate với system message chứa {context}
# và human message chứa {question}.
...
def build_rag_chain(vectorstore):
    ...
    # TODO hoàn thành: Tạo retriever từ vectorstore, lấy k=3 tài liệu gần nhất.
    retriever = vectorstore.as_retriever(search_kwargs={"k": 3})

    # TODO hoàn thành: Ghép page_content của các docs thành một context string.
    def format_docs(docs):
        return "\n\n".join(doc.page_content for doc in docs)

    # TODO hoàn thành: Xây dựng LCEL chain theo pipe retriever → prompt → LLM → parser.
    chain = (
        {"context": retriever | format_docs, "question": RunnablePassthrough()}
        | RAG_PROMPT
        | llm
...
# TODO hoàn thành: Decorator traceable giúp mỗi lần gọi tạo một trace riêng.
...
def ask(chain, question: str) -> str:
    ...
    """
    # TODO hoàn thành: Gọi chain.invoke(question) và trả về kết quả.
...
def main():
    ...
    # TODO hoàn thành: Gọi setup_vectorstore() để tạo vectorstore.
    vectorstore = setup_vectorstore()

    # TODO hoàn thành: Gọi build_rag_chain(vectorstore) để nhận chain và retriever.
    chain, retriever = build_rag_chain(vectorstore)

    # TODO hoàn thành: Lặp qua tất cả SAMPLE_QUESTIONS, gọi ask(), in câu hỏi và câu trả lời.
